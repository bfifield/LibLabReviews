...
use_aclu_issue_template <- function(type = "analysis", repo = "."){
  ...
  purrr::walk(c(issue_files,issue_config), ~ fs::file_copy(path = .x, new_path = issue_template_path))
...
use_aclu_label_template <- function(type = "analysis", repo = "."){
  ...
  type <- match.arg(type)
  conf_file <- yaml::read_yaml(file = file.path(issue_resources(type = type),"config.yml"))
  add_github_folder(repo = repo)
  label_template_path <- file.path(rep,".github","LABEL_TEMPLATE")
  fs::dir_create(path = label_template)
  fs::file_copy(path = conf_file, new_path = label_template_path)
